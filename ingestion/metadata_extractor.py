# ==========================================
# File: ingestion/metadata_extractor.py
# ==========================================
...
class MetadataExtractor:
    ...
    @staticmethod
    def extract(pdf_path: str,user_id: str) -> PDFMetadata:
        try:
            ...
            if not ContentValidator.is_valid_metadata(metadata):
                raise MetadataExtractionError("Metadata validation failed.")
            logger.info( f"Metadata extracted "f"successfully from "f"{pdf_path.name}")

            return metadata

        except Exception as e:

            logger.error(f"Metadata extraction failed "f"for {pdf_path}: {e}")
